...
if not r.status_code == 200:
    raise Exception(f'Status code not 200. Status code was: {r.status_code}. Response was: {r.text}')
...
if not r.headers.get('X-Server-Auth-Signature') == signature_check:
    print(f"Server signature: {r.headers.get('X-Server-Auth-Signature')}")
    print(f"Calculated signature: {signature_check}")
