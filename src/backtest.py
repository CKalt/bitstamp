...
# Parameters to tweak
FORECAST_HORIZON = 60  # Forecast 10 minutes into the future
PRICE_CHANGE_THRESHOLD = 0.5  # Threshold for significant price change
STOP_LOSS = -10  # Stop loss threshold
TAKE_PROFIT = 100  # Take profit threshold
...
def backtest_hw_strategy(data, timestamps):
    ...
    for i in range(len(data) - FORECAST_HORIZON):
        ...
        if not holding and price_difference > PRICE_CHANGE_THRESHOLD:
            ...
            if price_difference < -PRICE_CHANGE_THRESHOLD or exit_reason != "Forecast":
